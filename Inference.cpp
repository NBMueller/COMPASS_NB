...
Inference::Inference(std::string start_tree, std::string name, double temperature, int index):
        cache_scores{new Scores()},
        t{start_tree, false},
        t_prime{t},
        best_tree{t},
...
Tree Inference::find_best_tree(bool use_CNV, int nb_steps, int burn_in) {
    ...
    if (index >= 0) {
        std::cout << "Chain " << std::to_string(index) << ": Starting first phase (finding the best tree without CNV)." << std::endl;
    } else {
        std::cout << "Starting first phase (finding the best tree without CNV)." << std::endl;
    }
    mcmc(false, nb_steps, burn_in);
    if (!use_CNV) {
        if (tree_name != "") {
            best_tree.to_dot(tree_name + ".gv");
        }
        std::cout << "BEST LOG SCORE: " << best_tree.log_score << std::endl;
        return best_tree;
    }
    ...
    if (tree_name != "") {
        best_tree.to_dot(tree_name + "_noCNV.gv");
    }
    // Find best tree with CNV
    if (index >= 0) {
        std::cout << "Chain " << std::to_string(index) << ": Starting second phase (finding the best tree with CNV)." << std::endl;
    } else {
        std::cout << "Starting second phase (finding the best tree with CNV)." << std::endl;
    }
    best_tree.allow_CNV();
    t = best_tree;
    t_prime = t;
...
void Inference::mcmc(bool use_CNV, int nb_steps, int burn_in) {
    ...
    for (int step=0; step < nb_steps; step++) {
        if (parameters.verbose) {
            std::cout << "MCMC step " << step << "  ----------------------------------------" << std::endl;
        }
        ...
        if (step > burn_in && use_CNV) {
            max_move_index = move_weights.size();
        }
        ...
        switch(move_id) {
            case 0:
                if (parameters.verbose) {
                    std::cout << "Selected prune and reattach" << std::endl;
                }
                t_prime.prune_reattach();
                break;
            case 1:
                if (parameters.verbose) {
                    std::cout << "Selected swap node labels" << std::endl;
                }
                t_prime.swap_node_labels();
                break;
            case 2:
                if (parameters.verbose) {
                    std::cout << "Selected move mutation" << std::endl;
                }
                t_prime.move_mutation();
                break;
            case 3:
                if (parameters.verbose) {
                    std::cout << "Selected split/merge node" << std::endl;
                }
                t_prime.split_merge_node();
                break;
            case 4:
                if (parameters.verbose) {
                    std::cout << "Selected add/remove CNLOH" << std::endl;
                }
                t_prime.add_remove_CNLOH();
                break;
            case 5:
                if (parameters.verbose) {
                    std::cout << "Selected move CNLOH" << std::endl;
                }
                t_prime.move_CNLOH();
                break;
            case 6:
                if (parameters.verbose) {
                    std::cout << "Selected add/remove CNV" << std::endl;
                }
                t_prime.add_remove_CNV();
                break;
            case 7:
                if (parameters.verbose) {
                    std::cout << "Selected move CNV" << std::endl;
                }
                t_prime.move_CNV();
                break;
            case 8:
                if (parameters.verbose) {
                    std::cout << "Selected merge or duplicate CNV" << std::endl;
                }
                t_prime.merge_or_duplicate_CNV();
                break;
            case 9:
                if (parameters.verbose) {
                    std::cout << "Selected exchange CNV/CNLOH" << std::endl;
                }
                t_prime.exchange_CNV_CNLOH();
                break;
            case 10:
                if (parameters.verbose) {
                    std::cout << "Selected change alleles affected by CNV" << std::endl;
                }
        ...
        if (parameters.verbose) {
            std::cout << "t_prime score: " << t_prime.log_score << ", t score: "
            << t.log_score << ", hastings ratio:" << t_prime.hastings_ratio
            << ", acceptance ratio:" << acceptance_ratio << ",priorT "
            << t.log_prior_score << ", priorT' " << t_prime.log_prior_score << std::endl;
        }
        ...
        if (rd <= acceptance_ratio) {
            ...
            // reject t_prime
            if (parameters.verbose) {
                std::cout << "Rejected move" << std::endl;
            }
            t_prime = t;
        }
